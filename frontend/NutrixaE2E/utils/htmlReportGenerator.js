...
<body>
  ...
  if (process.env.GITHUB_STEP_SUMMARY) {
    ...
    let summaryMarkdown = `### 🧬 Nutrixa E2E Clinical Audit Pipeline Summary
...
`;
    ...
    Object.keys(typeMap).forEach(type => {
      const data = typeMap[type];
      summaryMarkdown += `| ${type} | ${data.total} | ${data.passed} | ${data.failed} |\n`;
    });
...
`;
    ...
    results.forEach(res => {
      const statusText = res.status === 'PASS' ? '🟢 PASS' : '🔴 FAIL';
      summaryMarkdown += `| ${statusText} | ${res.category} | ${res.type} | ${res.name} | ${res.duration}ms | ${res.error || ''} |\n`;
    });

    fs.appendFileSync(summaryPath, summaryMarkdown);
    console.log(`📝 Appended pipeline step summary with tables to GITHUB_STEP_SUMMARY.`);
  }
}
